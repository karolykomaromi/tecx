namespace Cars.CodeQuality.Test
{
    ...
    public abstract class RuleTests
    {
        ...
        protected void AssertCodeViolatesRule(string codeFileName, Type rule)
        {
            ...
            {
                Console.WriteLine(Properties.Resources.RuleNotViolated, rule.Name);

                throw;
            }
        }

        protected void AssertCodeDoesNotViolateRule(string codeFileName, Type rule)
        {
            this.AddSourceCode(codeFileName);
            this.StartAnalysis();
            this.WriteViolationsToConsole();
        ...
        private void WriteViolationsToConsole()
        {
            foreach (var violation in this.violations)
            {
                ...
                Console.WriteLine(
                    Properties.Resources.ViolationsMessage,
        ...
        private void AddSourceCode(string resourceFileName)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(Properties.Resources.SourceFileCouldNotBeAdded, resourceFileName));
            }
        }
