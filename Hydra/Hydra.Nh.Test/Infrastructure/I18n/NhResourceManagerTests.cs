namespace Hydra.Nh.Test.Infrastructure.I18n
{
        ...
        public void Should_Find_ResourceItem_By_Name_And_Culture(ISession session)
        {
            using (session)
            {
                using (var tx = session.BeginTransaction())
                {
                    ...
                    Assert.Equal("1", sut.GetString(TypeHelper.GetPropertyName(() => Properties.Resources.Foo), culture));
                    Assert.Equal("2", sut.GetString(TypeHelper.GetPropertyName(() => Properties.Resources.Bar), culture));
